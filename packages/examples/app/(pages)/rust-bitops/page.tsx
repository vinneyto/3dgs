...
import { getRustWasm } from "@/app/_shared/lib/rustWasm";
...
export default function RustBitOpsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const pows = Array.from(
          mod.powers_of_two_u32(a) as unknown as number[],
        );
  ...
  return (
    <div className="page">
      <div className="pageHeader">
        <h1>Rust shift-right playground</h1>
        <div className="muted">
          File: <code>app/(pages)/rust-bitops/page.tsx</code>
        </div>
        <p className="muted">
      ...
      {state.kind === "loading" && (
        <div style={{ marginTop: 12 }}>Generating report…</div>
      )}
      {state.kind === "error" && (
      ...
      {state.kind === "ready" && (
        <>
          ...
          <div style={{ marginTop: 8 }}>
            <b>set_bit_u32(a, k)</b>: {setResult} (
            <code>0x{setResult.toString(16).toUpperCase()}</code>)
          </div>
          <div style={{ marginTop: 8 }}>
          ...
          <div style={{ marginTop: 8 }}>
            <b>Powers of two in a</b>: <code>[{powers.join(", ")}]</code>
          </div>
          <pre
            style={{
              marginTop: 12,
