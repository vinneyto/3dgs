...
import {
  RADIX_SORT_THREE_TSL_DEMO_N,
  RADIX_SORT_THREE_TSL_DEMO_WORKGROUP_SIZE_X,
  compareCpuArraySortToGpu,
  disposeRadixSortThreeTslBenchmark,
  prepareRadixSortInput,
  runRadixSortCpuArraySort,
  runRadixSortThreeTslGpu,
  type RadixSortWebgpuDemoResult,
} from "./runRadixSortThreeTslSetup";

const BYTES_PER_U32 = 4;

export default function RadixSortThreeTslDemoPage() {
  const [busy, setBusy] = useState(false);
  const [phase, setPhase] = useState<string | null>(null);
  const [result, setResult] = useState<RadixSortWebgpuDemoResult | null>(null);
  const [error, setError] = useState<string | null>(null);

  const bytes = useMemo(() => RADIX_SORT_THREE_TSL_DEMO_N * BYTES_PER_U32, []);
  ...
  return (
    <div style={{ padding: 16, maxWidth: 980 }}>
      <h2 style={{ margin: "0 0 8px" }}>three.js + TSL radix-sort demo</h2>
      <p style={{ margin: "0 0 12px", opacity: 0.85 }}>
        Runs the same 16-pass (2-bit) radix pipeline as the WebGPU demo, but
        dispatched through three.js WebGPU renderer + TSL/wgslFn compute nodes.
      </p>
      ...
      >
        ...
        <div>
          {RADIX_SORT_THREE_TSL_DEMO_N.toLocaleString()}{" "}
        ...
        <div>
          {RADIX_SORT_THREE_TSL_DEMO_WORKGROUP_SIZE_X}×1×1{" "}
          ...
          onClick={async () => {
            ...
            try {
              ...
              const prepared = prepareRadixSortInput({
                n: RADIX_SORT_THREE_TSL_DEMO_N,
                seed: 123456789,
              });

              flushSync(() => setPhase("Running GPU radix sort (three.js+TSL)..."));
              const { result: gpuResult, gpuSorted } = await runRadixSortThreeTslGpu({
                prepared,
              });
