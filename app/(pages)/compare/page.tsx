...
import { createCovarianceEllipsoidNodes } from "@/app/_shared/tsl/covarianceEllipsoid";
import {
...
} from "@/app/_shared/tsl/gaussian/helpers";
import { unpackCovariance3D } from "@/app/_shared/tsl/gaussian/covarianceMath";
import { WebGPUCanvasFrame } from "@/app/_shared/webgpu/WebGPUCanvasFrame";

export default function SplatComparePage() {
  const {
  ...
  const [splat] = useState(() => {
    ...
    const vPosition = corner
      .mul(sqrt(max(cutoffA, 1e-8)))
      .toVarying("vPosition");
    ...
    const mask = float(uShowQuadBg).select(
      1.0,
      float(rgba.w).div(max(float(uOpacity), 1e-6)),
    );
  ...
  return (
    <div className="page">
      <div className="pageHeader">
        ...
        <div className="muted">
          File: <code>app/(pages)/compare/page.tsx</code>
        </div>
      </div>
